...
public class SSHService {
    ...
    private String getCMD(Field[] fields_tmp,EdgeServerCmd cmd_tmp) throws IllegalArgumentException, IllegalAccessException, StreamReadException, DatabindException, IOException{
        String cmdTotal = "";
        for(Field field : fields_tmp){
            field.setAccessible(true); 
            cmdTotal += (String) field.get(cmd_tmp) + " ; "; 
        }
        return cmdTotal;
    }
    ...
    private void getLinesByCMDWithChannel(EdgeServer edgeServer, Session session, String command, Field[] fields)
    ...
    public EdgeServer getDataOfEdgeServer(String host, String user, String password, String ID){
        ...
        try {
            ...
            command = getCMD(fields,cmd);
            session = getJsch(host, user, password);

            getLinesByCMDWithChannel(edgeServer, session, command, fields);
